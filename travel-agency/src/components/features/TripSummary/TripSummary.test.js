...
//czy poprawnie renderują się propsy name, cost i days,
    it ('should render correct props', () => {
        ...
        const expecedCost = 'cost';
        const expectedDays = 1;
        const component = shallow(<TripSummary id='abc' image='image.jpg' name={expectedName} cost={expecedCost} days={expectedDays} tags={[]} />)

        const renderedName = component.find('.title').text();
        expect(renderedName).toEqual(expectedName);

        const renderedCost = component.find('span').text(); // dziecko  do details, jak się odnieść?
        expect(renderedCost).toEqual(expectedCost);
        
        const renderedDays = component.find('span').text(); //// dziecko  do details, jak się odnieść?
...
//czy renderowane są tagi
    it('should render tags', () => {
        ...
        expect(renderedTag).toEqual(expectedTags);
    })
